...
def _substitute(node: Any, prompt: str) -> Any:
    """Troca %prompt% DENTRO dos valores ja parseados.

    Substituir no JSON cru quebraria com aspas ou barra invertida no prompt,
    que vem de song.toml e e' texto livre do autor.
    """
...
def build_graph(template: str, prompt: str, seed: int, prefix: str) -> dict:
    ...
    graph = _substitute(graph, prompt)
...
def _submit(base: str, graph: dict) -> str:
    body = json.dumps({"prompt": graph, "client_id": "keyframes_gen"}).encode()
...
def _wait(base: str, pid: str, timeout_s: int) -> list[str]:
    """Devolve os arquivos REALMENTE produzidos. status nao e' evidencia."""
    t0 = time.time()
...
def main() -> int:
    ...
    for i, job in enumerate(jobs, 1):
        ...
        pid = _submit(base, graph)
        files = _wait(base, pid, args.timeout)
        if not files:
            print(f"  [{i}/{len(jobs)}] {job['id']}: SEM SAIDA (status nao e' evidencia)")
            continue
        src = args.output_dir / files[0]
        if not src.exists():
            print(f"  [{i}/{len(jobs)}] {job['id']}: declarado {files[0]} mas nao esta em disco")
            continue
